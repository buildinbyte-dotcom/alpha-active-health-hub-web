# Gemini.md - Project Context & Memory

## Project Overview
**Client:** Alpha Active Health Hub
**Current URL:** `https://alphaactivehealth.com.au/`
**Goal:** Modernize the website, improve UX for older patients, add a new specialist (Physiotherapy), and introduce a "Recommended Products" revenue stream.

## Key Information
*   **Location:** Level 3, Suite 306, Alpha Building, 5 Celebration Drive, Bella Vista NSW 2153.
*   **Phone:** (02) 9189 1008
*   **Fax:** (02) 8905 9625
*   **Principal Doctor:** Dr. Shyamini Gunaratne (MBBS, FRACP) - Consultant Rheumatologist.
*   **Services:** Rheumatology, Autoimmune conditions, Musculoskeletal care.
*   **New Addition:** Physiotherapy Specialist (Profile to be added).

## Constraints & Requirements
*   **Target Audience:** Older adults/seniors.
    *   *Strict Requirement:* WCAG Accessibility (Large fonts, high contrast, easy navigation).
*   **Quality Assurance:**
    *   *Strict Requirement:* All new features and components **MUST** include unit tests.
    *   *Strict Requirement:* Zero regressions allowed. Run tests before every major change.
*   **Tech Stack:** Headless/Modern architecture.
*   **Management:** Non-technical admin must be able to update content (Team, Products, Articles).
*   **Cost:** Minimize ongoing costs (Free tiers preferred).
*   **Features:**
    *   **Wellness Recommendations:** Doctor-recommended products with affiliate links. **MUST include Disclaimer Component.**
    *   **Health Resources:** Articles/guides for patient education (SEO focus).
    *   **Booking:** NO internal booking engine. "Book Now" links to Phone or Contact Page.
    *   **Forms:** Use Web3Forms for contact submissions (No serverless complexity).

## Extracted Content (Do Not Lose)
*   **Dr. Bio:** "Highly accomplished physician and rheumatologist... Staff Specialist at Macarthur Health Service... Dual training in rheumatology and nuclear medicine..."
*   **Philosophy:** "Expert care meets compassionate service... Patient-centred approach."
*   **Parking/Location Info:** "Visitor parking spots available on-site... Level 3, Suite 306..."
